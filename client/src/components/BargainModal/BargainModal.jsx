...
const BargainModal = ({
  ...
  const handleSubmit = () => {
    ...
    if (offerPrice > product.discountPrice) {
      toast.error("Offer price cannot exceed the listed price");
      return;
    }
    if (offerPrice < product.discountPrice * 0.4) {
      toast.error("Offer price is too low");
      return;
    }
    if (offerPrice === product.discountPrice) {
      toast.error("Offer must be lower than the listed price");
      return;
    }
      ...
      <div className="p-6">
        ...
        <div className="flex gap-3 bg-[#FBF8F5] rounded-3xl p-3.5 mb-5 border border-[#E8D4D0]/50">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-[#2C1810] line-clamp-2 leading-snug font-['Outfit']">
              {product.name}
            </p>
            {selectedSize && (
            ...
            <p className="text-sm font-bold text-[#E7A9A2] mt-1 font-['Outfit']">
        ...
        {/* Quick Suggestions */}
        <div className="mb-4">
          <p className="text-[10px] font-bold text-[#8A6B65] uppercase tracking-widest mb-2.5">
            Quick Suggestions
          </p>
            ...
            {[80, 70, 60].map((percent) => (
              ...
              >
                ...
                <span className="font-normal opacity-75">
                  ₹{Math.floor((product.discountPrice * percent) / 100)}
                </span>
              </button>
            ))}
          </div>
        ...
        <div className="mb-5">
          ...
          <p className="text-[11px] text-[#8A6B65] mt-1.5">
            Offer range: ₹{Math.floor(product.discountPrice * 0.4)} – ₹
