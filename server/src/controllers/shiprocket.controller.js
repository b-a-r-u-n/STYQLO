...
const checkServiceability = asyncHandler(async (req, res) => {
  ...
  if (!checkedPinCode) throw new apiError(500, "Error while checking pin code");
...
const getCourierDetails = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json(
      new apiResponse(
        200,
        "Courier details fetched successfully",
...
const generateAWB = asyncHandler(async (req, res) => {
  ...
  if (!courierId) throw new apiError(400, "Courier ID is required");
  ...
  if (awbResponse?.awb_assign_status !== 1)
    throw new apiError(
      500,
      awbResponse?.response?.data?.awb_assign_error ||
        "Error while assigning recommended courier",
    );

  const awb = awbResponse?.response?.data;

  order.shiprocket.awbCode = awb?.awb_code || null;
  ((order.shiprocket.courierCompanyId = awb?.courier_company_id
    ? String(awb.courier_company_id)
    : null),
    (order.shiprocket.courierName = awb?.courier_name || null),
    (order.shiprocket.status = "AWB Generated"));
  ...
  res
    .status(200)
    .json(new apiResponse(200, "AWB generated successfully", data));
});
...
const generateShipmentLabelAndInvoice = asyncHandler(async (req, res) => {
  ...
  order.shiprocket.invoiceUrl = invoice?.invoice_url;
  order.shiprocket.labelUrl = label?.label_url;
  ...
  res
    .status(200)
    .json(
      new apiResponse(200, "Label and invoice generated successfully.", {
...
const requestPickup = asyncHandler(async (req, res) => {
  ...
  if (!shipmentId) throw new apiError(400, "Shiprocket shipment not found");
  ...
  if (
    result?.response?.pickup_scheduled_date &&
  ...
  res
    .status(200)
    .json(new apiResponse(200, "Pickup requested successfully", result));
});
...
const createManifest = asyncHandler(async (req, res) => {
  ...
  if (order?.shiprocket?.pickupStatus !== "SCHEDULED")
    throw new apiError(
      400,
      "Pickup must be scheduled before generating manifest.",
    );
  ...
  res
    .status(200)
    .json(new apiResponse(200, "Manifest generated successfully", result));
});
...
const createReturnShipment = asyncHandler(async (req, res) => {
  ...
  const returns = await Return.findById(returnId)
    .populate(
      "products.product",
      "-_id -description -originalPrice -discountPrice -stock -images -star -createdAt -updatedAt -__v",
    )
    .populate("user", "email -_id")
    .populate("order", "paymentMethod shippingAddress -_id")
    .select(
      "-createdAt -__v -method -refundPayment -refundedAt -receivedAt -rejectedAt -requestedAt -approvedAt -refundStatus -returnStatus -products.returnedQuantity",
    );
  ...
  if (!shiprocketResponse)
    throw new apiResponse(
      500,
      "Error while creating Shiprocket return shipment",
    );
...
const getReturnCourierOptions = asyncHandler(async (req, res) => {
  ...
  if (!returnId) throw new apiError(400, "Return ID is required");

  const returns = await Return.findById(returnId).populate(
    "order",
    "paymentMethod shippingAddress -_id",
  );

  if (!returns) throw new apiError(404, "Return not found");
  ...
  if (!serviceability)
    throw new apiError(
      500,
      "Error while checking return courier serviceability",
    );
...
const assignReturnAWB = asyncHandler(async (req, res) => {
  ...
  returns.shiprocket.awbCode = awb?.awb_code || null;
  returns.shiprocket.courierCompanyId = awb?.returns.shiprocket
    .courier_company_id
    ? String(awb.courier_company_id)
    : null;

  returns.shiprocket.courierName = awb?.courier_name || null;
  returns.shiprocket.status = "AWB Generated";
  returns.shiprocket.pickupStatus = "SCHEDULED";
  returns.shiprocket.pickupScheduledDate = awb?.pickup_scheduled_date;
  ...
  res
    .status(200)
    .json(new apiResponse(200, "Return AWB generated successfully", data));
});
...
const shiprocketWebhook = asyncHandler(async (req, res) => {
  try {
    ...
    if (!data.awb) {
      console.log("Shiprocket webhook received without AWB");
    ...
    if (!order) {
      console.log(`No STYQLO order found for AWB: ${data.awb}`);
    ...
    order.shiprocket.tracking.shipmentStatusId =
      data.shipment_status_id || null;

    order.shiprocket.tracking.currentTimestamp = parseShiprocketDate(
    ...
    for (const scan of incomingScans) {
      ...
      const alreadyExists = existingScans.some((existing) => {
        const sameDate =
          existing.date &&
          scanDate &&
          existing.date.getTime() === scanDate.getTime();

        const sameActivity = existing.activity === scanActivity;

        const sameLocation = existing.location === scanLocation;

        return sameDate && sameActivity && sameLocation;
      });
      ...
      if (!alreadyExists) {
        existingScans.push({
          ...
          srStatus: scan["sr-status"] || null,

          srStatusLabel: scan["sr-status-label"] || null,
        });
      }
    }
...
export {
  ...
  getCourierDetails,
  generateAWB,
  generateShipmentLabelAndInvoice,
  requestPickup,
  createManifest,
  shiprocketWebhook,
